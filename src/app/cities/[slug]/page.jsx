...
export async function generateMetadata({ params }) {
  const product = await fetch(`${process.env.BACKEND_URL}/cities/${params.slug}`,{
    headers : {
...
const page = async ({params}) => {
  const post = await fetch(`${process.env.BACKEND_URL}/courses/${params.slug}/cities`,{
    headers : {
      'Content-Type': 'application/json',
      'Accept-Language' : 'en'
    },
  ...
  const specialization = await fetch(`${process.env.BACKEND_URL}/specializations`,{
    headers : {
      'Content-Type': 'application/json',
      'Accept-Language' : 'en'
    },
  ...
  return (
    <div>
        ...
        <SearchFilters_cities post={post} params={params} specialization={specialization} Category={Category}/>
